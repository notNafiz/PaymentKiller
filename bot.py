...
def normalize_domain_url(url: str) -> str:
    ...
    https://dainte.com/ -> https://dainte.com/
    """
    url = re.sub(r'^(http:\/\/|https:\/\/)?(www\.)?', '', url, flags=re.IGNORECASE)
...
async def load_bot_settings_from_supabase():
    ...
    try:
        ...
        if response.data:
            settings = {item['setting_name']: item['setting_value'] for item in response.data}
        ...
        print(f"CRITICAL ERROR: Failed to load bot settings from Supabase: {e}")
        return False

async def update_bot_setting_in_supabase(setting_name: str, setting_value: str):
    try:
        ...
        print(f"CRITICAL ERROR: Supabase update bot setting {setting_name} failed: {e}")
        return False
...
async def add_pk_domain_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not DOMAIN or not PK:
        ...
        if not DOMAIN or not PK:
            ...
            await update.message.reply_text(escape_markdown(
                "Bot settings (PK/DOMAIN) are not loaded. Please contact an admin.", version=2),
                                            parse_mode='MarkdownV2')
            return
...
async def post_init(application: Application):
    ...
    await load_bot_settings_from_supabase()
    if not DOMAIN or not PK:
        print("WARNING: PK or DOMAIN could not be loaded at startup. Bot may not function correctly.")
        print("Please ensure 'bot_settings' table exists and has 'PK' and 'DOMAIN' entries, and RLS allows 'anon' role to read.")
